...
- **16 个渗透 Skill** — 7 核心 + 9 专项 Skill，含 111 个参考文档
...
| **Agent 核心** | `agent/core.py`                                  | OpenAI SDK + Tool Calling + 自主渗透循环 + think 过滤 |
...
> 共 11 个 MCP 服务、23 个工具定义。另有 2 个内置 Agent 工具（`load_skill_reference` + `crypto_decode`），无需 MCP 即可调用。
...
| crypto-toolkit            | 2          | 编解码/加解密（29 种操作，注册为内置工具）   |
...
| v0.2     | 移动端能力（Frida / ADB / JADX）— Skill 已就绪          | 🔜 Skill ✅ |
